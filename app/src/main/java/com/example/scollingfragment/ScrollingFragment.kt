...
import com.example.scollingfragment.databinding.FragmentScrollingBinding
...
class ScrollingFragment : Fragment(R.layout.fragment_scrolling) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val binding = DataBindingUtil.bind<FragmentScrollingBinding>(view)
        ...
        binding?.nameShow?.text = myVM.fullName.value
        binding?.username?.setText(passedInfo[7])
        binding?.usernameShow?.text = myVM.username.value
        binding?.email?.setText(passedInfo[8])
        binding?.emailShow?.text = myVM.email.value
        binding?.password?.setText(passedInfo[9])
        binding?.passwordShow?.text = myVM.password.value
        binding?.rePassword?.setText(passedInfo[4])
        binding?.genderShow?.text = myVM.gender.value
        ...
        binding?.btnHideInfo?.setOnClickListener {
            ...
            parentFragmentManager.commit {
                ...
                replace<WriteFragment>(R.id.fragmentHolder, args = info)
            }
        }
